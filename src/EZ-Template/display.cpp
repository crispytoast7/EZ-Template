...
// portrait screen is LLEMU's theme at a different size, not a lookalike. The
// four lv_style_t's LLEMU adds (frame_style, screen_style, button_style,
// lv_style_transp_fit) live in .bss and are never initialized, so every visible
// property comes from these calls plus the default theme.
...
constexpr lv_coord_t LINE_PITCH = 18;
constexpr lv_coord_t PANEL_INSET = 14;
constexpr lv_coord_t BAR_HEIGHT = 30;
constexpr lv_coord_t BTN_WIDTH = 76;
constexpr lv_coord_t MARGIN = 11;
...
lv_obj_t* llemu_button_search(lv_obj_t* parent, int index, int& found) {
  uint32_t children = lv_obj_get_child_cnt(parent);
...
// LLEMU's three buttons are the only lv_btn objects it builds, in btn0/btn1/
...
void portrait_button_cb(lv_event_t* event) {
  ...
  if (target != nullptr) lv_event_send(target, code, nullptr);
}

void portrait_create() {
  lv_coord_t w = lv_disp_get_hor_res(nullptr);
  lv_coord_t h = lv_disp_get_ver_res(nullptr);
  lv_coord_t panel_w = w - (MARGIN * 2);
  lv_coord_t panel_h = h - BAR_HEIGHT - (MARGIN * 4);
  ...
  lv_obj_clear_flag(g_portrait_screen, LV_OBJ_FLAG_SCROLLABLE);
...
void repaint_schedule() {
  lv_timer_t* timer = lv_timer_create([](lv_timer_t* t) { lv_obj_invalidate(lv_scr_act()); }, 60, nullptr);
...
void portrait_unload() {
  ...
  if (g_llemu_screen != nullptr) lv_scr_load(g_llemu_screen);
  repaint_schedule();
}
}  // namespace
...
void screen_line_set(int line, std::string text) {
  ...
  if (g_portrait_on) {
    if (line >= 0 && line < LINE_COUNT) lv_label_set_text(g_portrait_lines[line], text.c_str());
    return;
  }
...
void screen_line_clear(int line) {
  ...
  if (g_portrait_on) {
    if (line >= 0 && line < LINE_COUNT) lv_label_set_text(g_portrait_lines[line], "");
    return;
  }
...
void screen_lines_clear() {
  ...
  if (g_portrait_on) {
    for (int i = 0; i < LINE_COUNT; i++) lv_label_set_text(g_portrait_lines[i], "");
    return;
  }
  pros::lcd::clear();
}
